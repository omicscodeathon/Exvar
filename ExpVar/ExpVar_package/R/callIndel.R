#' Call insertion/deletion variants
#' 
#' This function calls indel variants from BAM files. The results are formatted 
...
#' @param SNPlocs An SNPlocs object containing dbSNP IDs. For indels, this may be an XtraSNPlocs object.
#' @param threads The number of cores to use. Cores should equal a factor of reference genome sequence levels ie. chromosome contigs should be equally divisible between cores.
#' @param outputdir The output directory for the VCF file.
#' @return A list of file paths to the VCF files.
#' @export
callIndel <- function(bam,
                      genome,
                      genomedir,
                      SNPlocs,
                      threads,
  ...
  if (class(genome) != "GmapGenome") {
    ...
    if (exists(threads)) {
      if ((length(seqlevels(refgen))) %% threads != 0) {
        stop(paste0("Threads should be a multiple of ", 
                    length(seqlevels(refgen)), "."))
      }
    } else {
      for(i in 3:length(seqlevels(refgen))) {
        if((length(seqlevels(refgen)) %% i) == 0) {
          threads <- i
          break
        }
      }
    ...
    for (i in bamfl) {
      ...
      sampleNames(indels) <- file_path_sans_ext(basename(path(i)))
      mcols(indels) <- NULL
      print("Formatting variant information as VCF...")
      vcf <- asVCF(indels)
      writeVcf(vcf, 
               paste0(file_path_sans_ext(basename(path(i))), "_indels.vcf"), 
               index = TRUE)
      vcf <- readVcf(paste0(file_path_sans_ext(basename(path(i))), "_indels.vcf.bgz"))
