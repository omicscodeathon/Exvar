...
##Sets the reference genome that corresponds to the species chosen by the user
  ...
  control <- getReadCountsFromBAM(control_bamfl, basename(control_bamfl), seqlevels(control_bamfl[1]), parallel = 10,)
  print("Experiment")
  #experiment <- countBamListInGRanges(countWindows = countWindows,
   #                                   bam.files = experiment_bamfl, 
    #                                  read.width = FALSE)
  experiment <- getReadCountsFromBAM(experiment_bamfl, basename(experiment_bamfl), seqlevels(experiment_bamfl[1]), parallel = 10)
